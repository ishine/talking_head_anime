...
class BlendDataset(BaseDataset):
    def __init__(self, conf):
        ...
        for path_dir in self.dirs:
            files = self.find_all_blends(path_dir)
    ...
    def getitem(self, idx):
        ...
        for shape_key, value in random_shapekeys.items():
            try:
                Renderer.change_shapekey(shape_key, value)
        ...
        for obj_key, obj in bpy.data.objects.items():
            ...
            if hasattr(obj, 'pose') and hasattr(obj.pose, 'bones') and obj.pose.bones is not None:
                # rotate head
                for pose_key, value in random_poses.items():
                    Renderer.rotate_bone(obj, [('頭', pose_key, value)])
                bpy.ops.object.mode_set(mode='OBJECT')
