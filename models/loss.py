...
class THAVGGLoss(torch.nn.Module):
    def __init__(self, resize=True):
        super(THAVGGLoss, self).__init__()
        blocks = []
    ...
    def forward(self, input, target, feature_layers=[0, 1, 2, 3], style_layers=[]):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in feature_layers:
                lambda_i = 0.2 * 4 * input.shape[-2] * input.shape[-1] / (x.shape[-3] * x.shape[-2] * x.shape[-1])
                losses[f'feat_{i}'] = F.l1_loss(x, y) * lambda_i
            if i in style_layers:
                act_x = x.reshape(x.shape[0], x.shape[1], -1)
                act_y = y.reshape(y.shape[0], y.shape[1], -1)
                gram_x = act_x @ act_x.permute(0, 2, 1)
                gram_y = act_y @ act_y.permute(0, 2, 1)
                losses[f'style_{i}'] = F.l1_loss(gram_x, gram_y)
        return losses
